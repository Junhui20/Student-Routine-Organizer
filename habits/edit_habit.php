...
if ($habit_id <= 0 || !habit_belongs_to_user($habit_id)) { header("Location: index.php"); exit(); }
...
if (!$habit) { header("Location: index.php"); exit(); }
...
<div class="container">
  ...
  <form method="POST" action="">
    ...
    <div class="form-group">
      ...
      <input type="text" name="habit_name" required value="<?= htmlspecialchars($_POST['habit_name'] ?? $habit['habit_name']) ?>">
    </div>
    ...
    <div class="form-group">
      ...
      <select name="frequency">
        <option value="daily" <?= (($_POST['frequency'] ?? $habit['frequency'])==='daily')?'selected':''; ?>>Daily</option>
        <option value="weekly" <?= (($_POST['frequency'] ?? $habit['frequency'])==='weekly')?'selected':''; ?>>Weekly</option>
        <option value="custom" <?= (($_POST['frequency'] ?? $habit['frequency'])==='custom')?'selected':''; ?>>Custom</option>
      </select>
    </div>
...
<div id="timer_block" style="display: <?= (($_POST['habit_type'] ?? '')==='timer') ? 'block' : 'none' ?>;margin-top:1rem;">
  <div class="form-group" style="padding:1rem;border:1px solid #ddd;border-radius:8px;background:#f9f9f9;">
    ...
    <div class="form-group">
      ...
      <input type="date" name="start_date" required value="<?= htmlspecialchars($_POST['start_date'] ?? $habit['start_date']) ?>">
    </div>
